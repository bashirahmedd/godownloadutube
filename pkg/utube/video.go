...
func youtubeClient(cfg *cutils.ConfigVideo, yt *youtubeVideo) bool {
	...
	log.Printf("Target video size : %d", size)
	defer stream.Close()
...
func writeBufferedStream(dst io.Writer, src io.Reader, buf []byte, szdl int) (written int64, err error) {
	...
	for {
		...
		for retrycnt:=10; retrycnt > -1 ; retrycnt-- {
			nr, er = src.Read(buf)
			if er!=nil  && cutils.MatchPatternInVal("*connection reset by peer*", er.Error()){
		...
		log.Printf("Read Status: num: %d status: %v", nr, er)
		if er == nil {
			totBytes += int64(nr)
			log.Printf("In progress size : %d  remaining: %d \n", totBytes, szdl - int(totBytes))
			bar.Add(nr)
			if nr > 0 {
